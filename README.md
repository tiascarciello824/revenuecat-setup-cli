# 🚀 RevenueCat Setup CLI

Automated RevenueCat project setup for React Native and Expo apps. Create projects, configure apps, set up products, and generate integration code in minutes.

## ✨ Features

- 🔐 **OAuth Login** - Browser-based authentication (no API key copy-paste!)
- 🤖 **Auto-Detection** - Reads `app.json` for Bundle ID and Package Name
- 📱 **Auto App Creation** - Creates iOS and Android apps automatically
- 🛍️ **Product Setup** - Configures subscriptions with trials
- 🎯 **Entitlements & Offerings** - Complete offering configuration
- 📝 **Code Generation** - TypeScript integration code ready to use
- 🔄 **Supabase Integration** - Optional webhook and database setup
- 🔑 **API Key Auto-Write** - Updates `.env` automatically

## 🎯 What It Does

1. **Auto-detects** your app configuration from `app.json`
2. **Creates RevenueCat project** (or uses existing)
3. **Creates iOS and Android apps** with your Bundle IDs
4. **Retrieves API keys** automatically from RevenueCat
5. **Writes keys to `.env`** - No manual copy-paste!
6. **Creates products**: Monthly and Annual subscriptions with 7-day trials
7. **Configures entitlements** and offerings
8. **Generates integration code**: Service layer, store, types
9. **Supabase setup** (optional): Webhook function and SQL migration

## 🚀 Quick Start

### Prerequisites

- Node.js 16+
- React Native or Expo project
- RevenueCat account

### Installation

```bash
# Clone the repository
git clone https://github.com/tiascarciello824/revenuecat-setup-cli.git
cd revenuecat-setup-cli

# Install dependencies
npm install

# Build
npm run build
```

### Usage

#### Smart Mode (Recommended)

```bash
cd /path/to/your/react-native-project
node /path/to/revenuecat-setup-cli/dist/index.js --smart
```

Or create an alias:

```bash
# Add to ~/.zshrc or ~/.bashrc
alias revenuecat-setup="node /path/to/revenuecat-setup-cli/dist/index.js"

# Then use it anywhere
cd your-project
revenuecat-setup --smart
```

## 🔐 Authentication

### OAuth Login (Recommended)

If OAuth is configured, you'll see:

```
🔑 Autenticazione RevenueCat

? Scegli il metodo di autenticazione:
  ❯ 🌐 Login con browser (OAuth) - Consigliato
    🔑 API Key manuale
```

Select OAuth to:
1. Open browser automatically
2. Login to RevenueCat
3. Authorize the app
4. ✨ Done! No copying API keys

### API Key (Fallback)

If OAuth is not available, you'll be prompted for your RevenueCat Secret API Key:
- Find it in RevenueCat Dashboard → Settings → API Keys
- Use the **Secret Key** (starts with `sk_`)

## 📋 What You'll Get

### Generated Files

```
your-project/
├── lib/
│   ├── services/
│   │   └── revenuecat.ts          # RevenueCat service layer
│   └── supabase/
│       └── subscriptions.sql       # Database migration
├── store/
│   └── subscriptionStore.ts        # Zustand subscription store
├── types/
│   └── subscription.ts             # TypeScript types
├── supabase/
│   └── functions/
│       └── handle-revenuecat-webhook/
│           └── index.ts            # Webhook handler
└── .env                            # Updated with API keys ✨
```

### Updated .env

```bash
# Auto-generated by the CLI
EXPO_PUBLIC_REVENUECAT_IOS_KEY=appl_xxxxxxxxxxxxx
EXPO_PUBLIC_REVENUECAT_ANDROID_KEY=goog_xxxxxxxxxxxxx
```

## 🎬 Demo Flow

```bash
$ revenuecat-setup --smart

🚀 RevenueCat Setup - Smart Mode
ℹ Rilevamento automatico della configurazione...

📱 Configurazione Rilevata
ℹ Nome App: MyAwesomeApp
ℹ Bundle ID iOS: com.company.app
ℹ Package Name Android: com.company.app
ℹ Platform: expo
ℹ Backend: supabase

🔑 Autenticazione RevenueCat
? Scegli il metodo di autenticazione: 🌐 Login con browser

🔓 OAuth callback server started
✓ Browser opened. Please authorize the application.
✓ Authorization successful!

🏗️  RevenueCat Project
✓ Progetto creato: MyAwesomeApp

📱 Creazione App
✓ App iOS creata
✓ App Android creata
✓ Chiavi API recuperate

⚙️  Creazione in RevenueCat
✓ Creati 2 prodotti
✓ Creati 1 entitlement(s)
✓ Creati 1 offering(s)

🔑 Aggiornamento .env
✓ Chiavi aggiunte al file .env

📝 Generazione Codice
✓ Generati 7 file(s)

✅ Setup Completato!
```

## 🛠️ Configuration

### Environment Variables

```bash
# OAuth Client ID (optional, for OAuth flow)
export REVENUECAT_OAUTH_CLIENT_ID="your_client_id"

# API Key (for API key auth)
export REVENUECAT_API_KEY="sk_xxxxxxxxxxxxx"
```

### Auto-Detection

The CLI automatically detects from `app.json`:
- App name
- iOS Bundle ID (`expo.ios.bundleIdentifier`)
- Android Package Name (`expo.android.package`)
- Platform (Expo vs React Native)
- Backend (Supabase, Firebase, etc.)

## 📦 Products Created

By default, creates:

- **Monthly Subscription**
  - ID: `{app}_pro_monthly`
  - Trial: 7 days
  - Type: Subscription

- **Annual Subscription**
  - ID: `{app}_pro_annual`
  - Trial: 7 days
  - Type: Subscription

- **Entitlement**: `pro` (includes both products)
- **Offering**: `default` (current, with both packages)

## 🔧 Advanced Usage

### Interactive Mode

```bash
revenuecat-setup --interactive
```

Customize every detail:
- Product IDs and pricing
- Trial periods
- Entitlement names
- Offering configuration

### Auto Mode

```bash
revenuecat-setup --auto
```

Zero prompts - uses all defaults.

## 🤝 Contributing

Contributions are welcome! Please feel free to submit a Pull Request.

## 📄 License

MIT

## 🆘 Support

- [RevenueCat Documentation](https://www.revenuecat.com/docs)
- [OAuth Setup Guide](./OAUTH_SETUP.md)
- [Issues](https://github.com/tiascarciello824/revenuecat-setup-cli/issues)

## ⚙️ OAuth Setup

To enable OAuth authentication:

1. Contact support@revenuecat.com to register an OAuth client
2. Set the `REVENUECAT_OAUTH_CLIENT_ID` environment variable
3. See [OAUTH_SETUP.md](./OAUTH_SETUP.md) for details

## 🎉 Credits

Built with ❤️ for the React Native and Expo community.

---

**Made by**: Carmine Mattiascarciello  
**Powered by**: [RevenueCat](https://www.revenuecat.com)
